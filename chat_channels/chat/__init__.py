...
class CRest:
    ...
    def installApp(self, request: WSGIRequest):
        ...
        if params['event'] == 'ONAPPINSTALL' and not PHP.empty(params['auth']):
    ...
    def setAppSettings(self, settings, isInstall = False):
        ...
        if isinstance(settings, (list, dict)):
            ...
            if isInstall != True and not PHP.empty(oldData) and isinstance(oldData, (list, dict)):
    ...
    def getAppSettings(self):
        ...
        if not PHP.empty(data.get('access_token')) and \
                not PHP.empty(data['domain']) and \
                not PHP.empty(data['refresh_token']) and \
                not PHP.empty(data['application_token']) and \
                not PHP.empty(data['client_endpoint']):
            isCurrData = True
    ...
    def getSettingData(self):
        ...
        if os.path.exists(file_path):
            ...
            if not PHP.empty(C_REST_CLIENT_ID):
                _return['C_REST_CLIENT_ID'] = C_REST_CLIENT_ID

            if not PHP.empty(C_REST_CLIENT_SECRET):
    ...
    def callRequest(self, params):
        ...
        if settings != False:
            ...
            try:
                if not PHP.empty(params.get('params')):
                    method = "POST"
                else:
                    method = "GET"

                headers = {
                    'User-Agent': 'Bitrix24 CRest PHP ' + str(self.VERSION),
                }
                ...
                if not PHP.empty(result.get('error')):
                    if result.get('error') == 'expired_token' and PHP.empty(params.get('this_auth')):
                        ...
                        if not PHP.empty(arErrorInform.get(result['error'])):
